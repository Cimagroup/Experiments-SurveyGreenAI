        ...
        for file in files:
            if file.endswith('.txt'):
                ...
                with open(path_file, 'r') as archivo:
                    ...
                    # Category 1: Only one object, person with no mobility problem
                    if len(lines) == 1 and (lines[0].startswith('0') or lines[0].startswith('2')):
                        category.append(0)
                    # Category 2: Only one object, person with mobility impairment
                    elif len(lines) == 1 and (lines[0].startswith('1') or lines[0].startswith('3') or lines[0].startswith('4')):
                        category.append(1)
                    # Category 3: Combination of objects, all persons without mobility problems.
                    elif len(lines) > 1 and all(line.startswith('0') or line.startswith('2') for line in lines):
                        category.append(2)
                    # Category 4: Combination of objects, all persons with mobility impairment
                    elif len(lines) > 1 and all(line.startswith('1') or line.startswith('3') or line.startswith('4') for line in lines):
                        category.append(3)
                    # Category 5: Combination of objects, people with and without mobility impairments
                    else:
                        category.append(4)
    return category
...
def fes(paths_images,perc,tensor_YOLO,category,args):
    ...
    tracker.start()
    init=time.time()
    indexes = train_fes(tensor,torch.tensor(trainLabels),model,criterion,optimizer,argsFES,perc)
    print("Estimated emissions: ", tracker.stop()*1000, " CO2 grams")
    end = time.time()
    elapsed_time = end - init
...
def main():
    ...
    parser.add_argument('--epochFES', default='10', type=int, help="Epoch of FES model train", required=False)
    ...
    if method not in posibblesMethods:
        ...
        if perc < 0 or perc > 1:
            ...
            archivos = os.listdir(path_new)
            for archivo in archivos:
                if archivo.endswith(".png") and archivo in paths_images_selected:
                    path_file = os.path.join(path_new, archivo)
                    path_file_new = os.path.join(path_folder, archivo)
                    shutil.copy(path_file,path_file_new)
